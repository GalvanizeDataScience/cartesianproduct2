...
object ReverseGeocode {
  ...
  def QueryDistinctLatLongs() : Seq[Map[String, Double]] = {
    ...
    val rawGeoData: CassandraTable[CassandraRow] = sc.cassandraTable("CartesianProduct", "geodictionary").select("latitude", "longitude") //CassandraTableScanRDD

    val coordinates = rawGeoData.map(x => Map("latitude" -> x(0), "longitude" -> x(1))).collect()
  coordinates
  }
  ...
  def Geocoder(coordinates: Seq[Map[String, Double]]) : CassandraTable = {
    ...
    val resultsSeq = for (point <- coordinates) yield parseJSON(GoogleMapsRequester(point("latitude"), point("longitude"), "AIzaSyCSnYlXKQbAmxoCZ4y8xtdD-JfCwADEuXI"))
    ...
    val geodataseq = for (geo <- geoSeq) yield new geodata(geo(0), geo(1), geo(2))
